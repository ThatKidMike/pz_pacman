...
public class Teleport : MonoBehaviour {
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if(/*collision.name == "watman_1" &&*/ gameObject.name == "right_portal") {
            ...
            Vector2 v_left = new Vector2((int)Math.Round(left.transform.position.x)+1, (int)Math.Round(left.transform.position.y));
            ...
            Vector2 v_right = new Vector2((int)Math.Round(right.transform.position.x)-1, (int)Math.Round(right.transform.position.y));
